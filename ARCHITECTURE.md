...
`BrowserExtension` uses temporary page access to extract editor text and UTF-16 ranges, draw browser underlines, and apply edits through the browser’s undo-preserving editing command. Its background context binds each editor session to its originating tab and shares one native connection with the toolbar popup. Reconnection requests fresh snapshots instead of replaying editor text or edits. Explicit popup connections may launch the registered app; background retries do not. `TextWardenBrowserHost`, embedded in the app, relays bounded native-messaging frames over a private, same-user Unix socket to `BrowserIntegration`. The host verifies its launching browser’s vendor signature and derives Chrome, Brave, Firefox, or Zen identity from that process and overwrites any browser identity in extension input; the app binds each connection to that browser for focus, pause, and replacement routing.

`BrowserIntegration` reuses the grammar engine, filters, preferences, Foundation Models, and native popover views. It owns browser sessions and revision checks; `AnalysisCoordinator` suspends AX monitoring only while the extension owns the active browser editor or a page the user explicitly paused. Paused pages retain a text-free ownership session so the native fallback does not reactivate underneath the page pause. Other fields and applications retain their existing AX path. Replacements require matching text and revision, and AI replacements also require the original selection. The extension honors canceled `beforeinput` events and revalidates before editing.

The toolbar popup owns page controls and reads saved website rules from the Mac app through validated configuration messages. Page activation runs in the background context so it survives the popup closing. The editor indicator shares page state with the popup and uses the native app’s appearance preference.

Browser replacements acknowledge the resulting text and revision before the native popover advances. Shared popover ownership and AX suppression prevent simultaneous native/browser grammar UI. Exact page and origin rules live in the existing native website settings, preserving non-default ports and path case while dropping query strings and fragments.

The browser pill receives native dimensions, colors, visibility, and hover preferences. Underline hover uses the native suggestion popover; dragging snaps to viewport edges. Pill right-click opens the toolbar popup, which owns page controls and the app’s existing pause settings. `Scripts/browser-extension.py` derives manifest versions from `Info.plist` and packages only runtime files for the release workflow.

`BrowserSetup` derives Chrome’s fixed extension ID from the bundled manifest’s public key and registers its exact origin and helper path for the current user at app startup. Unchanged registration is left intact; missing or stale registration is repaired automatically. No user-entered ID is needed. The Browser settings tab reports a live connection rather than inferring extension installation from an idle browser. Firefox and Zen use the fixed Gecko extension ID and per-browser native messaging registration. Safari embeds a signed, sandboxed app extension and uses a private app-group socket; requests and replies remain scoped to the originating extension context. Both socket endpoints require the same user and an expected TextWarden code signature. Chromium and Firefox receive separate bundled folders and release manifests; Safari ships inside the app. Google Docs and complex editor adapters remain outside this preview. See [the preview guide](BrowserExtension/README.md).
