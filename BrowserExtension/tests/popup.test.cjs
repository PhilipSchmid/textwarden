...
async function harness({ noReceiver = false, url = "https://example.com/editor", manifest = { version_name: "0.5.2", version: "0.5.2.38" } } = {}) {
  ...
  vm.runInNewContext(readFileSync(require.resolve("../popup.js"), "utf8"), {
    chrome, URL, crypto: { randomUUID: () => "popup-session" },
    document: { getElementById: element, querySelectorAll: selector => selector === "[data-pause]" ? pauses : [], documentElement: { dataset: {} } },
  ...
  return { element, timers, sent, port, reply, chrome };
}
function assertUnavailable(h) {
  for (const id of ["enabled", "underlines", "website", "settings"]) assert.equal(h.element(id).disabled, true, id);
}
...
test("healthy status stays quiet and website pauses stay visible", async () => {
  const h = await harness(); h.reply();
  assert.equal(h.element("status").hidden, true);
