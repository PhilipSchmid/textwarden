...
private struct OutlookTextPart: BoundedTextPart {
    let text: String
...
class OutlookStrategy: GeometryProvider {
    ...
    private func findBoundsForRange(_ targetRange: NSRange, in textParts: [OutlookTextPart], fullText _: String, element: AXUIElement) -> CGRect? {
        ...
        Logger.debug("OutlookStrategy: Calculated sub-bounds for \(overlappingParts.count) TextParts for range \(targetRange)", category: Logger.ui)
        return unionBounds
    }
